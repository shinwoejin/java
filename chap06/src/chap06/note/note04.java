...
public class note04 {
	...
	// Java API == 
	// Java Packages( == Class, Interfaces, Enum, Exception, 
	// Error,  Annotation 들의 모음)
	// Class Libraries 
	// java 9버전 이후에 모듈 개념 등장 : Java Modules(연관된 packages)
		
	// java.lang 안에 있는것은 import 없이 사용가능
	// Java API
	// - 클래스 라이브러리
	// - 프로그램 개발에 자주 사용되는 클래스 및 인터페이스 모음
	// - API 도큐먼트로 원하는 API 쉽게 찾아 이용할 수 있음
	// HashMap 사이즈는 정하지 않아도 사용가능, 하지만 양이 많아지면 느려짐.
	// - 문자열을 비교할때 == 연산자를 사용할 경우 원하지 않은 결과가 나올수 있음
